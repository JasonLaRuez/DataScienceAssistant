...
def test_each_problem_is_detected(messy):
    ...
    # 'joined' is a date hiding in a string: coerce_datetime (tier 1), then seen as a
    # real datetime column and proposed for extraction (tier 2) against the hypothetical
    # post-repair frame.
    assert "coerce_datetime" in kinds(plan.repairs)
    extraction = next(p for p in plan.transforms if p.kind == "extract_datetime_parts")
