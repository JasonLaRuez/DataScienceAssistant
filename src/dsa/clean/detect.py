    ...
    if datetimes:
        transforms.append(
            Proposal(
                ...
                summary=f"drop {len(datetimes)} datetime column(s)",
                evidence="; ".join(f"{c.name} ({c.n_unique:,} distinct timestamps)" for c in datetimes[:5]),
                consequence=(
                    "scikit-learn estimators cannot consume datetimes directly, and v1 "
                    "does no date feature engineering"
                ),
                alternatives=(
                    "derive year / month / day-of-week features in step 4",
                    "derive an elapsed-time feature relative to a reference date",
                ),
            )
        )
